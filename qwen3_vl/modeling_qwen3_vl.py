...
class Qwen3VLVisionModel(Qwen3VLPreTrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor, task: Optional[str] = None, **kwargs) -> torch.Tensor:
        """
        Args:
...
class Qwen3VLTextModel(Qwen3VLPreTrainedModel):
    ...
    def _deepstack_process_batch(
        self, hidden_states: torch.Tensor, visual_pos_masks: torch.Tensor, visual_embeds: torch.Tensor,
        deepstack_visual_batch_mask: torch.Tensor,
        layer_idx: int
    ):
        visual_pos_masks = visual_pos_masks.to(hidden_states.device)
        visual_embeds = visual_embeds.to(hidden_states.device, hidden_states.dtype)
        batch_indices = torch.arange(hidden_states.shape[0], device=hidden_states.device)
        active_mask = batch_indices[deepstack_visual_batch_mask > layer_idx]

        if len(active_mask) == 0:
            return hidden_states

        active_hidden = hidden_states[active_mask].clone()
        active_pos_masks = visual_pos_masks[active_mask]
        B, S, D = active_hidden.shape

        active_hidden_flatten = active_hidden.view(B * S, D)
        active_pos_mask_flatten = active_pos_masks.flatten()
        active_hidden_visual = active_hidden_flatten[active_pos_mask_flatten]
        active_hidden_visual = active_hidden_visual.view(B, -1, D)

        visual_embeds = visual_embeds.view(-1, active_hidden_visual.shape[1], active_hidden_visual.shape[2])
        bs = visual_embeds.shape[0]
        visual_embeds = visual_embeds.repeat(len(active_mask)//bs, 1, 1)

        local_this = active_hidden_visual + visual_embeds
        active_hidden_flatten[active_pos_mask_flatten] = local_this.view(-1, D)
        hidden_states[active_mask] = active_hidden_flatten.view(B, S, D)
...
class Qwen3VLModel(Qwen3VLPreTrainedModel):
    ...
    def get_video_features(
        self, pixel_values_videos: torch.FloatTensor, video_grid_thw: Optional[torch.LongTensor] = None, task_type: Optional[str] = None,
    ):
        """
        ...
        return self.get_image_features(pixel_values_videos, video_grid_thw, task_type)

    def get_image_features(self, pixel_values: torch.FloatTensor, image_grid_thw: Optional[torch.LongTensor] = None, task_type: Optional[str] = None):
        """
        ...
        image_embeds, deepstack_image_embeds, code_idx, vq_loss = self.visual(pixel_values, grid_thw=image_grid_thw, task=task_type)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if pixel_values is not None:
            image_embeds, deepstack_image_embeds, code_idx, vq_loss = self.get_image_features(pixel_values, image_grid_thw, task_type)
        ...
        #     cache_position=cache_position,
        #     visual_pos_masks=visual_pos_masks,
        #     deepstack_visual_embeds=deepstack_visual_embeds[:1],
        #     **kwargs,
        # )

        # outputs_2 = self.language_model(
        #     input_ids=None,
        #     position_ids=position_ids,
        #     attention_mask=attention_mask,
        #     past_key_values=past_key_values,
        ...
        #     cache_position=cache_position,
        #     visual_pos_masks=visual_pos_masks,
        #     deepstack_visual_embeds=deepstack_visual_embeds[:2],
        #     **kwargs,
        # )

        # outputs_final = self.language_model(
        #     input_ids=None,
        #     position_ids=position_ids,
        #     attention_mask=attention_mask,
        #     past_key_values=past_key_values,
        ...
        #     cache_position=cache_position,
        #     visual_pos_masks=visual_pos_masks,
        #     deepstack_visual_embeds=deepstack_visual_embeds[:3],
        #     **kwargs,
        # )
...
class Qwen3VLForConditionalGeneration(Qwen3VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        vision_vocab_size = self.model.visual.vq.quantize.codebook.shape[0]
        self.vision_vocab_size = vision_vocab_size + 1
        self.vision_embedding = nn.Embedding(vision_vocab_size, hidden_size)
        self.vision_embedding.weight.data[:vision_vocab_size] = self.model.visual.vq.quantize.codebook.clone()
